...
function getEnumValueFromEnumType(enumType: EnumType,value: string){
    ...
    if(enumType === 'int32') {
        ...
        n = value.substring(0,value.indexOf('//')).replace(/[,\ ]+$/,'').replace(/\'([a-zA-Z]+)'/,'$1');
    } else {
...
async function createDefinitionsFile({
    ...
    const map = new Map<string,{
        ...
        name: string;
        comments: string[];
    }>(contents.substring(offset,eof).split('\n')
    .map(l => {
        if(l.indexOf('=') === -1){
            ...
            if(commentIndex !== -1){
                l = `${l.substring(0,commentIndex)} = ${enumIndex++}${l.substring(commentIndex)}`;
            }
        }
    ...
    cs.write('\n');
    cs.write(`void bindings::${outClassName}::Init(v8::Local<v8::Object> exports) {\n`, () => {
        cs.write('auto obj = Nan::New<v8::Object>();\n');
        for(const [k,v] of map){
            cs.write(`Nan::Set(obj,Nan::New("${k}").ToLocalChecked(),${getExpressionFromValue(v.id)});\n`);
        }
        cs.write(`Nan::Set(exports,Nan::New("${propertyName}").ToLocalChecked(),obj);\n`);
    },'}\n');
    cs.write(`bool bindings::${outClassName}::Decode(v8::Local<v8::Value> jsValue, ${enumExpression}& out) {\n`, () => {
        if(enumType === 'char') {
            ...
            cs.write('if(Nan::DecodeWrite(strValue.data(),length,jsValue) != length) {\n', () => {
                cs.write('return false;\n');
            },'}\n');
            cs.write(`auto value = std::string_view(strValue.data(),length);\n`);
        } else {
            cs.write('if(!jsValue->IsNumber()) return false;\n');
            cs.write(`const auto value = Nan::To<v8::Number>(jsValue).ToLocalChecked()->Value();\n`);
        }
        for(const [,v] of map){
            cs.write(`if(value == ${typeof v.id === 'string' ? `"${v.id}"` : v.id}) {\n`, () => {
    ...
    cs.write(`v8::Local<v8::Value> bindings::${outClassName}::Encode(${enumExpression} value) {\n`, () => {
        cs.write(`switch(value) {\n`, () => {
            for(const v of new Map([...map].map(x => [x[1].id,x[1]])).values()){
    ...
    await fs.promises.writeFile(path.resolve(__dirname,`${outClassName}.cpp`),cs.value());

    cs = new CodeStream();
    cs.write(`export interface ${outInterfaceName} {\n`, () => {
        console.log(map);
    ...
    await fs.promises.writeFile(path.resolve(__dirname,`${outClassName}.d.ts`),cs.value());
    ...
    await fs.promises.writeFile(path.resolve(__dirname,`${outClassName}.hpp`),cs.value());
}

(async () => {
    await Promise.all([
        createDefinitionsFile({
            file: path.resolve(__dirname,'./imebra/library/include/imebra/dicomDefinitions.h'),
            ...
            outInterfaceName: 'IDicomDefinitions',
            enumType: 'int32',
            enumExpression: 'imebra::tagId_t'
        }),
        createDefinitionsFile({
            file: path.resolve(__dirname,'./imebra/library/include/imebra/definitions.h'),
            ...
            outInterfaceName: 'ITagVRDefinitions',
        ...
        createDefinitionsFile({
            ...
            sourceIncludes: ['<imebra/definitions.h>'],
            headerIncludes: ['<nan.h>','<imebra/definitions.h>'],
            outInterfaceName: 'IAgeUnitDefinitions',
        ...
        createDefinitionsFile({
            ...
            sourceIncludes: ['<imebra/definitions.h>'],
            headerIncludes: ['<nan.h>','<imebra/definitions.h>'],
            outInterfaceName: 'ICodecTypeDefinitions',
